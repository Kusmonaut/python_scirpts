...
def get_loci(rec_times, towers, v, delta_d, max_d):
    ...
    # Tower that receives the tdoa = 0.
    first_tower = int(np.where(rec_times == 0)[0])
    # Iterate over all other towers.
    for j in [x for x in range(towers.shape[1]) if x!= first_tower]:
        # print('tower', str(first_tower), 'to', str(j))
        locus = get_locus(tower_1=(towers[0][first_tower],
                                   towers[1][first_tower]),
                          tower_2=(towers[0][j], towers[1][j]),
