...
def decode_diagnostics(payload: bytes, node_id: int, tag_id: int) -> Optional[Dict[str, Any]]:
    ...
    # 47-49 "pp_ampl"
    # 49-66/49-193 "acc_memory" -> bytewise 17-144 bytes
    ...
    DIAG_READ_SUPPORT = "<BL5s4hL5s5sHh5sHLh"
    DIAG_ACC_MEM = "<17B"
    DIAG_ADVANCED_ACC_MEM = "<145B"

    DIAG_HEADER_LENGTH = 49
    DAIG_LENGTH = 66
    DIAG_ADVANCED_LENGTH = 194
    DWT_DIAGNOSTIC_LOG_REV_5 = 5

    if len(payload) == DIAG_ADVANCED_LENGTH:
        # decode diagnostics
        (
            head,
            diag["status"],
        ...
        if len(payload) == DAIG_LENGTH:
            diag["raw_cir"] = struct.unpack(DIAG_ACC_MEM, payload[DIAG_HEADER_LENGTH:])
        if len(payload) == DIAG_ADVANCED_LENGTH:
            diag["raw_cir"] = struct.unpack(DIAG_ADVANCED_ACC_MEM, payload[DIAG_HEADER_LENGTH:])
