...
def on_message(client: MQTTClient, userdata, message: MQTTMessage):
    ...
    if _tag_id == TRACE_TAG:
        original_position = copy.deepcopy(position)
        # change tag uid to uid + 1
        position['id'] = '0' + hex(int(position['id'], base=16) + 1)[2:]
        ...
        for debugNode in position['tdoadebug']:
            ...
            if 'raw_position' not in debugNode:
                ...
                if '05a260c907c40a15' == position['id']:
                    print('node: ' + debugNode['uid'] + '   tdoa: ' + str(debugNode['tdoa']))
        ...
        for debugNode in position['tdoadebug']:
            if 'raw_position' not in debugNode:
                if not debugNode['tdoa'] == None:
                    tdoadebug.append(debugNode)

        # filterout all None tdoas and rawpositions
        position['tdoadebug'] = tdoadebug

        # change Floor id in message
        position['floorId'] = FLOOR_ID
        original_position['floorId'] = FLOOR_ID
        
        if len(position['tdoadebug']) >= MIN_NUMBER_OF_TDOA_NODES:
            # calc new positi on
            _calc_pos = calc_tag_position(position['tdoadebug'])

            # check if calc was successful
            if np.shape(_calc_pos) == (1, 3):
                position['pos']['x'] = _calc_pos[0][0] 
                position['pos']['y'] = _calc_pos[0][1]
                ...
                # publish new position and old position
                _topic = 'tag/' + position['id'] + '/position/' + str(FLOOR_ID)
                _payload = json.dumps(position)
                client.publish(_topic, _payload)

                if _tag_id == TRACE_TAG:
                    _topic = 'tag/' + original_position['id'] + '/position/' + str(FLOOR_ID)
                    _payload = json.dumps(original_position)
                    client.publish(_topic, _payload)
...
def find_cluster(modes, cluster_sets, tdoa):
    ...
    if len(modes) > 0 and len(cluster_sets) > 0:
        ...
        if (stdeviation * N_TH_STD) + modes[0] > tdoa and tdoa > modes[0] - (stdeviation * N_TH_STD):
